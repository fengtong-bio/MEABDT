...
library(ggpubr)
###-buffalo--###
buffalo_comparison <- list(c("Stomach","Intestine"),c("Rectum","Intestine"),c("Rectum","Stomach"))
#relative abundance of proteins
{
  CAZy.all <- read.table("../test_files/CAZy_all.txt",header = F,sep = "\t")
  colnames(CAZy.all) <- c("CAZy_ID","abundance","group","source")
  CAZy.all$group <- factor(CAZy.all$group,levels = c("Stomach","Intestine","Rectum"),ordered = T)
  CAZy.all$source <- factor(CAZy.all$source,levels = c("Rumen","Reticulum","Omasum","Abomasum","Jejunum","Cecum","Colon","Feces"),ordered = T)
  pdf("buffalo_CAZy_g.pdf")
  p.buffalo_CAZy <- ggplot(CAZy.all,aes(x=group,y=abundance,fill = group)) + geom_boxplot()+
    scale_fill_manual(values = c("#F0BA97","#C3C1D7","#A1D5C5")) + theme_bw()+
  ...
  pdf("buffalo_CAZy_s.pdf")
  p.buffalo_CAZy.s <- ggplot(CAZy.all,aes(x=source,y=abundance,fill = source)) + geom_boxplot()+
    scale_fill_jco() + theme_bw()+
    theme(panel.grid.major = element_blank(),
          panel.grid.minor = element_blank()) +
    facet_wrap(~CAZy_ID,nrow = 2,scales = "free") +
...
#percentage
data.merge.all <- read.table("../test_files/CAZy_percentage.txt",header = T,sep = "\t")
...
dev.off()
#proportion
CAZy_level1 <- read.table("../test_files/CAZy_level1.txt",header = F,sep = "\t")
